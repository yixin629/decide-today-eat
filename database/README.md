...
| 音乐播放器     | `songs`、`pinned_artists`、`music_sync_session` | `migrations/music-player-schema.sql` + `migrations/music-player-enhancements.sql` + `migrations/music-player-listen-together.sql` | 后两者依次补充喜欢/置顶、以及“一起听”共享播放状态（Realtime）；反应表情（鲜花等）用 Realtime 广播，无需建表 |
...
| 换装共享作品墙 | `dress_up_outfits`              | `migrations/dress-up-shared-gallery.sql`                         | 完整造型 JSONB、原子点赞、挑战排行和 Realtime；本机造型仍保存在 localStorage                                                      |
...
| `music-player-listen-together.sql` | “一起听”共享播放状态单行表和 Realtime         |
