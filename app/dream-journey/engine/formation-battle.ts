...
function applyDamage(units: FormationUnit[], targetIds: string[], damageFor: (target: FormationUnit) => number) {
  ...
  const nextUnits = units.map((unit) => {
    ...
    const damage = Math.min(unit.hp, Math.max(1, Math.round(damageFor(unit))))
...
function actionOrder(units: FormationUnit[]) {
  return units
    .filter((unit) => !unit.summoned)
    .sort((a, b) => b.speed - a.speed)
    .map((unit) => unit.id)
}

export function createFormationBattle(battle: BattleState, stats: HeroStats, skills: SkillState, lineup: PartnerId[]): FormationBattleState {
  const level = stats.level
  const allies: FormationUnit[] = [
    { id: 'hero', name: '逍遥少侠', side: 'ally', role: 'hero', slot: 0, hp: stats.hp, maxHp: stats.maxHp, attack: stats.attack + 22 + skills.attackLevel * 3, speed: 102, model: 'hero' },
    ...lineup.map((partnerId, index) => {
      ...
      return {
        ...
        hp: 92 + level * 15,
        maxHp: 92 + level * 15,
        attack: 20 + level * 5 + (partner.role === 'striker' ? 8 : 0),
        speed: 96 - index * 4,
        model: partnerId,
        partnerId,
      }
    }),
    { id: 'pet', name: battle.companion.name, side: 'ally', role: 'pet', slot: 4, hp: 70 + level * 12, maxHp: 70 + level * 12, attack: battle.companion.attack + 12, speed: 72, model: battle.companion.model },
  ]
...
function phaseTransition(state: FormationBattleState): { state: FormationBattleState; event: FormationEvent } | null {
  ...
  const guardHp = Math.round(boss.maxHp * 0.24)
...
export function advanceFormationBattle(state: FormationBattleState): { state: FormationBattleState; event: FormationEvent } {
  ...
  if (actor.side === 'ally') {
    ...
    if (actor.role === 'healer' && wounded && wounded.hp / wounded.maxHp < 0.76) {
      ...
      event = { actorId: actor.id, targetIds: [wounded.id], skillName: PARTNERS['moon-lotus'].skill, message: `${actor.name}施展月华莲灯，为${wounded.name}恢复 ${healed} 点气血。`, kind: 'heal', amounts: { [wounded.id]: healed } }
    } else if (target) {
      ...
      const applied = applyDamage(units, targetIds, (unit) => actor.attack * multiplier * (aoe ? 0.78 : 1) * (critical ? 1.6 : 1) * (actor.role === 'breaker' && unit.role === 'guard' ? 1.45 : 1))
      units = applied.units
      const skillName = actor.role === 'hero'
        ? '破军连斩'
        : actor.role === 'pet'
          ? '灵泡追击'
          : actor.partnerId ? PARTNERS[actor.partnerId].skill : '协同攻击'
      const total = Object.values(applied.amounts).reduce((sum, amount) => sum + amount, 0)
      event = { actorId: actor.id, targetIds, skillName, message: `${actor.name}施展${skillName}，对 ${targetIds.length} 个目标造成 ${total} 点伤害${critical ? '（暴击）' : ''}。`, kind: 'damage', amounts: applied.amounts, critical }
    } else {
    ...
    const total = Object.values(applied.amounts).reduce((sum, amount) => sum + amount, 0)
    event = { actorId: actor.id, targetIds, skillName, message: `${actor.name}施展${skillName}，造成 ${total} 点伤害。`, kind: 'damage', amounts: applied.amounts }
  }
...
export function formationResult(state: FormationBattleState): FormationBattleResult | null {
  ...
  return { outcome: state.outcome, heroHp: Math.max(1, hero?.hp ?? 1) }
}
