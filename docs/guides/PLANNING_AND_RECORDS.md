...
`database/setup/planning-records-setup.sql` 创建。
...
历史数据库可能来自
`database/migrations/legacy/planning-tables-bundle.sql`、
`database/migrations/legacy/planning-tables-compatibility-bundle.sql`
或早期综合修复，字段名不完全一致。这些 legacy 脚本不用于默认初始化；已有数据库先用
`database/diagnostics/check-planning-tables.sql` 查看字段，再阅读 [数据库脚本说明](../../database/README.md)。
